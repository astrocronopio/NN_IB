...
########################################################################################################
def init_matriz(p,N): return  np.sign(np.random.uniform(-1, 1., (p,N)))
...
def iterar_determ_dynamic(matrix_j, patron_i_mu, N_epochs, N):
	...
	#		h_i_mu = np.dot(matrix_j[unit],patron_i_mu)
	#		punto_fijo_i_mu[unit]= np.sign(h_i_mu)
	#		if punto_fijo_i_mu[unit] == patron_i_mu[unit]: flag+=1
...
########################################################################################################
def ejer_1():
	N_vec=[4000]#, 1000 , 2000, 4000]
	alpha_vec = [0.18]#, 0.14, 0.16, 0.18]
	N_epochs=1000
